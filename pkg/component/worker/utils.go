...
func CreateDirectKubeletKubeconfig(ctx context.Context, k0sVars *config.CfgVars) (string, error) {
	...
	if apiSpec.OnlyBindToAddress {
		...
		if err != nil {
			logrus.WithError(err).Warn("Failed to resolve localhost, falling back to 127.0.0.1")
			loopbackIP = net.IPv4(127, 0, 0, 1)
		}
		localAPIServer = net.JoinHostPort(loopbackIP.String(), strconv.Itoa(apiSpec.Port))
	}

	logrus.Debugf("Using direct local API server URL for kubelet: %s", localAPIServer)
	...
	logrus.Debugf("Wrote direct kubeconfig file: %s", directKubeconfigPath)
